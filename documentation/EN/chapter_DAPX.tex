...
\begin{algorithmic}[1]
	\Procedure{Auction Experiment}{$\mu, \sigma, F$}
		...
		\State $n  = 100000$ \Comment{n can be any large number}
		...
		\For{each $bid$ in bid\_data}\Comment{stop until the last bid in bid\_data}                    
			\If{$bid \geqslant p_{opt}$}
				\State $\text{OPT}(F)$ = $\text{OPT}(F)$ + $p_{opt}$ \Comment{bidder wins and pay  $p_{opt}$}
			\EndIf
			\If{$bid \geqslant p_{D}$}
				\State ${\text{REV}(F)}$ = ${\text{REV}(F)}$ + $p_{D}$ \Comment{bidder wins and pay  $p_{D}$}
			\EndIf
		\EndFor
